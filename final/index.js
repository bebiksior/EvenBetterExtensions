var f=window.Caido;var d=new Set,q1=({id:z,cssText:q})=>{if(d.has(z)||document.querySelector(`#${z}`))return;const Z=document.createElement("style");Z.id=z,Z.textContent=q,Z.classList.add("evenbetterapi-css-module"),document.head.appendChild(Z),d.add(z)},W=q1;var F1=`
.eb-checkbox__input {
    cursor: pointer;
...
`,p=()=>{W({id:"checkbox",cssText:F1});const z=document.createElement("div");return z.classList.add("eb-button__label"),z.innerHTML=`<div class="eb-checkbox">
...
`;document.addEventListener("keydown",(z)=>{if(z.key==="Escape")k()});var Y1=({title:z,content:q})=>{W({id:"evenbetterapi-modal",cssText:Q1});const Z=document.createElement("div");Z.classList.add("evenbetter-modal"),Z.innerHTML=`
...
}`,_1=(z,q,Z)=>{const F=document.createElement("div");F.classList.add("v-toast"),F.classList.add(`v-toast--${Z}`);const G=document.createElement("div");G.setAttribute("role","alert"),G.classList.add("v-toast__item"),G.classList.add(`v-toast__item--${q}`),G.classList.add(`v-toast__item--${Z}`),F.appendChild(G);const K=document.createElement("div");K.classList.add("v-toast__icon"),G.appendChild(K);const Q=document.createElement("p");return Q.classList.add("v-toast__text"),Q.textContent=z,G.appendChild(Q),F.classList.add("v-toast--fade-in"),F},x=(z)=>{const{message:q,type:Z,position:F,duration:G}=z;W({id:"eb-toast",cssText:J1});let K=document.querySelector(`.v-toast--${F}`);if(!K)K=document.createElement("div"),K.classList.add("v-toast"),K.classList.add(`v-toast--${F}`),document.body.appendChild(K);const Q=_1(q,Z,F);K.appendChild(Q),setTimeout(()=>{Q.classList.add("v-toast--fade-out"),setTimeout(()=>{Q.remove()},150)},G-150)};function l(z,q="Search...",Z=!1,F){W({id:"eb-text-input",cssText:X1});const G=document.createElement("div");G.classList.add("formkit-outer","c-text-input__outer"),G.setAttribute("style",`width: ${z};`);const K=document.createElement("div");K.classList.add("formkit-wrapper"),K.style.display="flex";const Q=document.createElement("div");Q.classList.add("formkit-inner","eb-text-input__inner");const Y=document.createElement("div");Y.classList.add("c-text-input__prefix"),Y.setAttribute("style","align-self: center; color: var(--c-fg-subtle);");const $=document.createElement("i");if($.classList.add("c-icon","fas"),F)$.classList.add(F);const _=document.createElement("input");if(_.setAttribute("placeholder",q),_.setAttribute("spellcheck","false"),_.classList.add("formkit-input","c-text-input__input"),_.setAttribute("type","text"),_.setAttribute("style","width: 100%; background: transparent; border: 0; outline: 0; color: inherit; box-sizing: border-box; line-height: inherit;"),F)Y.appendChild($);if(Q.appendChild(Y),Q.appendChild(_),Z){const V=document.createElement("button");V.innerHTML='<i class="fas fa-copy"></i>',V.setAttribute("style","background: transparent; border: 0; outline: 0; cursor: pointer; padding: 0; margin-left: 10px;"),V.addEventListener("click",()=>{navigator.clipboard.writeText(_.value),J.toast.showToast({message:"Copied to clipboard",position:"bottom",type:"info",duration:1000})}),Q.appendChild(V)}return K.appendChild(Q),G.appendChild(K),G}var X1=`
